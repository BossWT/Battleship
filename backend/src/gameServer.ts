	...
	private listen(): void {
		...
		this.io.on('connection', (socket: Socket) => {
			...
			this.active.push(
				new ActiveSocket(
					socket.id,
					setInterval(() => socket.emit('heartbeat', this.active.length), 5000)
				)
			);
			...
			socket.on('createRoom', (username: string) => {
				createRoom(socket, username, randomRoom(), address, this.roomList);
			});
			...
			socket.on('disconnect', () => {
				...
				if (
					this.active.find((conn) => conn.socketID === socket.id) != undefined
				)
					clearInterval(
						this.active.find((conn) => conn.socketID === socket.id).timer
					);
				this.active.splice(
					this.active.findIndex((conn) => conn.socketID === socket.id),
					1
				);
			});
