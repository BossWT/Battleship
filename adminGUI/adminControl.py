...
def adminControl(sio, instance):
    ...
    controlLayout = [
        [sg.Button("Get All Room"), sg.Text("User Online:"), sg.Text(key="mt2")],
    ...
    while True:
        ...
        if controlEvent == sg.WIN_CLOSED or controlEvent == "Exit":
            ...
            time.sleep(1)
            controlWindow["mt1"].update(
                "RoomID %-*s  Host Username %-*s  Guest Username\n" % (20, " ", 15, " ")
            )
            controlWindow["mt1"].print("-" * 75)
            for room in instance.roomList:
                if "guestUsername" in room:
                    controlWindow["mt1"].print(
                        "%-*s %-*s %s"
                        % (
                            30,
                            room["roomID"],
                    ...
                    controlWindow["mt1"].print(
                        "%-*s %-*s %s"
                        % (30, room["roomID"], 30, room["hostUsername"], "Not Set")
                    )
        ...
        controlWindow["mt2"].update(instance.count)
